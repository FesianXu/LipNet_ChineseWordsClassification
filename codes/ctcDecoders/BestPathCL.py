...
class CLWrapper:
    ...
    def __init__(
        ...
        self.queue = cl.CommandQueue(
            self.context, self.device)  # command queue to first GPU

        # buffer
        sizeOfFloat32 = 4
        batchBufSize = batchSize * maxC * maxT * sizeOfFloat32
        self.batchBuf = cl.Buffer(
            self.context,
            cl.mem_flags.READ_ONLY,
            size=batchBufSize,
            hostbuf=None)
        self.res = np.zeros([batchSize, maxT]).astype(np.int32)
        self.resBuf = cl.Buffer(
            self.context,
            cl.mem_flags.WRITE_ONLY,
            self.res.nbytes)
        self.tmpBuf = cl.Buffer(
            self.context,
            cl.mem_flags.WRITE_ONLY,
            self.res.nbytes)
        ...
        self.program = cl.Program(self.context, open(
            'BestPathCL.cl').read()).build(buildOptions)
    ...
    def compute(self, batch):
        ...
        cl.enqueue_write_buffer(
            self.queue, self.batchBuf, batch.astype(
                np.float32), is_blocking=False)

        # one pass
        if self.kernelVariant == 1:
            cl.enqueue_nd_range_kernel(
                self.queue, self.kernel1, (self.batchSize, self.maxT), (1, self.maxT))
        # two passes
        else:
            cl.enqueue_nd_range_kernel(
                self.queue,
                ...
                (1,
                 ...
                 self.maxC))
            cl.enqueue_nd_range_kernel(
                self.queue, self.kernel2, (self.batchSize,), None)
