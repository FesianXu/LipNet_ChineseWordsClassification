...
def ctcTokenPassing(mat, classes, charWords):
    ...
    # w' in paper: word with blanks in front, back and between labels: for ->
    # _f_o_r_
    ...
    while t <= maxT:
        ...
        for wIdx in sortedWordIdx:
            ...
            while s <= len(wPrime):
                ...
                if wPrime[s -
                          1] != blankIdx and s > 2 and wPrime[s -
                                                              2 -
                                                              1] != wPrime[s -
                                                                           1]:
            ...
            maxTok = sorted([toks.get(wIdx, len(wPrime), t), toks.get(
                wIdx, len(wPrime) - 1, t)], key=lambda x: x.score, reverse=True)[0]
    ...
    return str(' ').join([charWords[i]
                          for i in toks.get(bestWIdx, end, maxT).history])
