...
def recLabelingProb(t, s, mat, labelingWithBlanks, blank, cache):
    ...
    res = (recLabelingProb(t - 1,
                           s,
                           mat,
                           labelingWithBlanks,
                           blank,
                           cache) + recLabelingProb(t - 1,
                                                    s - 1,
                                                    mat,
                                                    labelingWithBlanks,
                                                    blank,
                                                    cache)) * mat[t,
                                                                  labelingWithBlanks[s]]

    # in case of a blank or a repeated label, we only consider s and s-1 at
    # t-1, so we're done
    if labelingWithBlanks[s] == blank or (
            s >= 2 and labelingWithBlanks[s - 2] == labelingWithBlanks[s]):
        cache[t][s] = res
        return res

    # otherwise, in case of a non-blank and non-repeated label, we
    # additionally add s-2 at t-1
    res += recLabelingProb(t - 1,
                           s - 2,
                           mat,
                           labelingWithBlanks,
...
def emptyCache(maxT, labelingWithBlanks):
    "create empty cache"
    return [[None for _ in range(len(labelingWithBlanks))]
            for _ in range(maxT)]
...
def ctcLabelingProb(mat, gt, classes):
    ...
    labelingWithBlanks = Common.extendByBlanks(Common.wordToLabelSeq(
        gt, classes), blank)  # ground truth text as label string extended by blanks
    # cache subresults to avoid recalculating  subproblems over and over again
    cache = emptyCache(maxT, labelingWithBlanks)
    return recLabelingProb(maxT - 1,
                           len(labelingWithBlanks) - 1,
                           mat,
                           labelingWithBlanks,
                           blank,
                           cache) + recLabelingProb(maxT - 1,
                                                    len(labelingWithBlanks) - 2,
                                                    mat,
                                                    labelingWithBlanks,
                                                    blank,
                                                    cache)
